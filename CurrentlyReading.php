...
/*
	...
	Plugin URI: https://blog.damn.org.za/widgets/
...
class CurrentlyReading extends WP_Widget {
    public function __construct() {
		$widget_ops = array(
			'classname' => 'reading_widget',
			'description' => 'Display a Currently Reading widget using the Google Books API',
			'internalcss' => true
		);
		parent::__construct( 'reading_widget', 'Currently Reading', $widget_ops );
	...
	function widget($args, $instance) {
		...
		$newtab      = $instance[ "newtab" ]      ? true : false;
		...
		$usetls      = $instance[ "usetls" ]      ? true : false;
		...
		if ($instance['isbn'] != "") {	  // No point in a "Currently Reading" if you aren't, is there?
			...
			if ($booksapi == true) {
				...
				if (file_exists($cache) and time() - filectime($cache) < $cachedays * 24 * 60 * 60 and filesize($cache) > 50) {
			...
			if ($booksapi == true and $isbnjson[ "totalItems" ] > 0) {
				...
				print("\t\t\t<a href='" . str_replace("google.com", $localdomain, $googlelink) . "'" . $tabextra . ">");
				print("<img class='currentlyreading' id='currenlyreading-ISBN" . $myisbn . "-img' ");
				print($shadow);
				$thumblink = $isbnjson[ 'items' ][0][ 'volumeInfo' ][ 'imageLinks' ][ 'thumbnail' ];
				if ($usetls == true) {
					$thumblink = str_replace("http:", "https:", $isbnjson[ 'items' ][0][ 'volumeInfo' ][ 'imageLinks' ][ 'thumbnail' ]);
				}
				print("src='"   . $thumblink										 	. "' ");
				...
				print("\t\t\t<a href='" . $uri . "://books." . $localdomain . "/books?vid=ISBN$myisbn'" . $tabextra . ">");
				print("<img class='currentlyreading' id='currenlyreading-ISBN" . $myisbn . "-img' ");
				print($shadow);
				print("src='" . $uri . "://books." . $localdomain . "/books?vid=ISBN$myisbn&printsec=frontcover&img=1&zoom=1' ");
	...
	function update($new_instance, $old_instance) {
		...
		$instance[ 'newtab' ]	    = $new_instance[ 'newtab' ]      ? 1 : 0;
		...
		$instance[ "usetls" ] 		= $new_instance[ "usetls" ]      ? 1 : 0;
	...
	function form($instance) {
		...
		$newtab      = $instance[ 'newtab' ]      ? "checked='checked'" : "";
		...
		$usetls      = $instance[ 'usetls' ]      ? "checked='checked'" : "";
		...
		print("<title>Deselecting this will attempt to brute-force the Google Books lookup</title></label>\n\t<br />");

		print("\t\t<input class='checkbox' type='checkbox' " . $usetls);
		print(" id='" . $this->get_field_id("usetls") . "' name='" . $this->get_field_name("usetls") . "'/>\n");
		print("\t\t<label for='" . $this->get_field_id("usetls") . "'>"); _e("Force all calls to use 'https'");
		print("<title>Use 'https'. Deselecting this will connect using 'http' and will not force 'https' links when using the API.</title></label>\n\t<br />");

		print("\t\t<input class='checkbox' type='checkbox' " . $newtab);
		print(" id='" . $this->get_field_id("newtab") . "' name='" . $this->get_field_name("newtab") . "'/>\n");
		print("\t\t<label for='" . $this->get_field_id("newtab") . "'>"); _e("Open Link in a New Tab/Window");
		print("</label>\n\t<br />\n");
...
//
function register_reading_widget() {
    register_widget( 'CurrentlyReading' );
}
add_action( 'widgets_init', 'register_reading_widget' );
